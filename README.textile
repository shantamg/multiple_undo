...
# Download the source, make sure the parent folder is called "multiple_undo", and place it in the "plugins" folder in your app.
# Create the database tables from the sql file located in /multiple_undo/config/schema/tables.sql.
# Add the following to the appController:
...
h4. Initializing the Undo Tables
...
h4. Now, Undo and Redo!

To undo or redo, you could put something like this in your view:

<pre><code><?=$html->link('undo', array('plugin' => 'multiple_undo', 'controller' => 'undos', 'action' => 'undo'));?>	
...
One thing that I did not mention is the implementation of the "description" of each undo. This description is optional, and could be kept in the session and displayed as the user clicks back through undos...
